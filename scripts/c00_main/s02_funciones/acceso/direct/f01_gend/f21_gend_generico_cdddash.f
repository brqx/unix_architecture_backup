...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E gendfolder a_syst   -- Entrara en la carpeta system
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
gendfolder()
{
IS_gend="Accede a una ruta filtrada con el comienzo del parametro con formato a_"
INFO_SCRIPT=${IS_gend}
...
CARPETA_DESTINO=$(echo "${SALIDA_codir}" | tr " " "\n" | grep "^a@${EXPR_CODIFICADA}") ;
SALIDA_gend=$?

CARPETA_DESTINO=$(echo "${SALIDA_codir}" | tr " " "\n" | grep "^a@${EXPR_CODIFICADA}" | tail -1 ) ;
