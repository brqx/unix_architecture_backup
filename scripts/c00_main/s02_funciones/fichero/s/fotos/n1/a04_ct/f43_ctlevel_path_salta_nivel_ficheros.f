...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E contlevel_path /opt  -- Genera una copia de los ficheros en /opt
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
e_pmt uplevel_path "[cadena] [ruta] [corte_cadena]"
...
EXTENSION=${ i::${LONGITUD_CADENA}-${CORTE_CADENA_ARCHIVO} }

NOMBRE_CONT=${cont}${EXTENSION}

cp $i ${RUTA_PASADA}/${CADENA_FINAL}-${NOMBRE_CONT}
