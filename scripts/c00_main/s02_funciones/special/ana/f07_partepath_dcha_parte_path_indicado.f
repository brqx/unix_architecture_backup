...
partepath_dcha()
{
IS_partepath_dcha="Parte el path indicado en razon a la parte pedida"
INFO_SCRIPT=${IS_partepath_dcha}
...
SALIDA_partepath_dcha=$( echo ${RUTA_COMPLETA} | cut -c1-${LONGITUD_PEDIDA} )

#-Hay que asegurarse que la cadena esta contenida

let LONGITUD_PEDIDA++

FINAL_PARTE_COMPLETA=$( echo ${RUTA_COMPLETA} | cut -c${LONGITUD_PEDIDA}-${LONGITUD_RUTA_COMPLETA} )

if [ ${FINAL_PARTE_COMPLETA} == ${PARTE_PASADA} ] ; then
## echo "Salida: ${SALIDA_partepath} "
return 0
else
echo "Error : ${FINAL_PARTE_COMPLETA} != ${PARTE_PASADA} "
return 1
fi
