...
#-------------------------------------------------------------------
#T codificacion listado carpetas code file spaces sudo
#-------------------------------------------------------------------
#+ Parametros : 
#-------------------------------------------------------------------
...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E sucodir X	       -- Codifica listado carpetas de la ruta actual con cambiando las X
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

#-Funcion para codificar un listado 
sucodir()
{
IS_sucodir="Codifica un listado carpetas para poder tratarse como super usuario"
INFO_SCRIPT=${IS_sucodir}
...
e_pmt sucodir "LINEA CON CARACTER GENERAL \"${CGE}\""

else

#- Filtramos solo las carpetas (undescores x GEN - spacios x underscores)
SALIDA_sucodir=$(sudo ls -1p | grep "/" | cut -d "/" -f1 | tr ${CSL} ${CGE} | tr "${CSP}" ${CSL} | tr "\n" " ")
