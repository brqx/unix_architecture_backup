...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E codcat FICHERO	       -- Codifica esa fichero para poder tratarse
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

#-Funcion para codificar
codcat()
{
IS_codcat="Codifica un fichero para poder tratarse"
INFO_SCRIPT=${IS_codcat}
...
e_pmt codcat "FICHERO_A_CODIFICAR [CARACTER GENERAL]"
...
e_pmt codcat "LINEA CON CARACTER GENERAL \"${CGE}\""

else

cat ${FICHERO} | tr ${CSL} ${CGE} | tr "${CSP}" ${CSL} 
