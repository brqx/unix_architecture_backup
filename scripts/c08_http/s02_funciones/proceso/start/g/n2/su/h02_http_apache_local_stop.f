...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E apache_local_stop s1 	-- Inicia el servicio indicado
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

apache_local_stop()
{
IS_apache_local_stop="Detiene el servidor web apache"
INFO_SCRIPT=${IS_apache_local_stop}

#-Guardamos la informacion actual del script. Es necesario si se invoca a scripts intermedios
INFO_SCRIPT_apache_local_stop=${INFO_SCRIPT}

RUTA_SERVIDORES=${1}

SALIDA_apache_local_stop=0

#-Se trata de un texto de informacion a devolver al llamante
INFOSALIDA_apache_local_stop=""

NOMBRE_SCRIPT_apache_local_stop=apache_local_stop
PARAMETROS_SCRIPT_apache_local_stop="[-?]"
...
e_pmt ${NOMBRE_SCRIPT_apache_local_stop} ${PARAMETROS_SCRIPT_apache_local_stop}
...
TIMESTAMP_apache_local_stop_01=${RETORNO_get_current_timestamp}
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--
...
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR++
http_su_stop_apache
SALIDA_apache_local_stop=${SALIDA_http_su_stop_apache}
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--
;;
'114' )
posiciona "Frontal:${NUMERO_SERVIDOR}-Deteniendo"
...
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR++
http_su_stop_apache
SALIDA_apache_local_stop=${SALIDA_httpsudo_stop_apache}
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--
;;
'201' )
posiciona "Core:${NUMERO_SERVIDOR}-Deteniendo"
...
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR++
get_current_timestamp
TIMESTAMP_apache_local_stop_02=${RETORNO_get_current_timestamp}
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--

#-Preparar la salida de la funcion

let SW_LLAMADA_DESDE_NIVEL_SUPERIOR++
stamp2date ${TIMESTAMP_apache_local_stop_01}
HORA_INICIAL=${RETORNO_stamp2date}
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--


let SW_LLAMADA_DESDE_NIVEL_SUPERIOR++
diffstamps ${TIMESTAMP_apache_local_stop_02} ${TIMESTAMP_apache_local_stop_01}
TIEMPO_TARDADO_01=${RETORNO_diffstamps}
TIEMPO_TARDADO_02=$(echo ${RETORNO_difffechas} | cut -d " " -f2 )
let SW_LLAMADA_DESDE_NIVEL_SUPERIOR--


if [ "${SALIDA_apache_local_stop}" == "0" ] ; then
#--------========--------========--------========--------========---
...
INFO_SCRIPT=${INFO_SCRIPT_apache_local_stop}
NOMBRE_SCRIPT=${NOMBRE_SCRIPT_apache_local_stop}
PARAMETROS_SCRIPT=${PARAMETROS_SCRIPT_apache_local_stop}

e_pms 

else
#- Si no se presenta se deben eliminar parametros del nivel actual
INFOSALIDA_apache_local_stop=${MOTIVO_FUNCION}
limpia_parametros
...
return ${SALIDA_apache_local_stop}
#-------------------------------------------------------------------
#- Fin de la funcion
#-------------------------------------------------------------------
fi
