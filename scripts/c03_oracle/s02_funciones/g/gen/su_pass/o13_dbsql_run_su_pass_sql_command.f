...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E run_su_pass_sql script pendient  -- Lanza la consulta SQL
#-------------------------------------------------------------------
#E run_su_pass_sql "$LINORA" " / as sysdba"  -- Lanza la consulta como sysdba
#-------------------------------------------------------------------
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
run_su_pass_sql()
{
IS_run_su_pass_sql="Ejecuta comando sql pasado como cadena como usuario externo a Oracle"
INFO_SCRIPT=${IS_run_su_pass_sql}

INFO_SCRIPT_run_su_pass_sql=${INFO_SCRIPT}
...
SALIDA_run_su_pass_sql=0

NOMBRE_SCRIPT_run_su_pass_sql=run_su_pass_sql
PARAMETROS_SCRIPT_run_su_pass_sql="CONEXION CADENA_BD"
...
e_pmt ${NOMBRE_SCRIPT_run_su_pass_sql} ${PARAMETROS_SCRIPT_run_su_pass_sql}
...
SALIDA_run_su_pass_sql=${SALIDA_oracle_parser}
...
##echo "ibm" | sudo -S su - oracle -c 'echo "conn cosym/cosym;:select * from dual;:quit" | tr ":" "\n" | sqlplus /nolog '
echo "ibm" | sudo -S su - oracle -c 'oracle_parser "${CADENA_BD}" | sqlplus ${CONEXION_BD}'
salida_sqlplus=$?

SALIDA_run_su_pass_sql=${salida_sqlplus}
...
INFO_SCRIPT=${INFO_SCRIPT_run_su_pass_sql}
NOMBRE_SCRIPT=${NOMBRE_SCRIPT_run_su_pass_sql}
PARAMETROS_SCRIPT=${PARAMETROS_SCRIPT_run_su_pass_sql}
