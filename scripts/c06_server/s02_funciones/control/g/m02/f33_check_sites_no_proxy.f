...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E check_sites_no_proxy	--> Chequea todos los servidores indicados
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
check_sites_no_proxy()
{
IS_check_sites_no_proxy="Comprueba los servidores. Puerto http."
INFO_SCRIPT=${IS_check_sites_no_proxy}

RUTA_SERVIDORES=$1

if [ "${RUTA_SERVIDORES}" == "" ] ;  then
RUTA_SERVIDORES=${RS_CONN}zsites_noproxy.dat
fi


if [ "${RUTA_SERVIDORES}" == "-?" ] ;  then

e_pmt check_sites_no_proxy "[RUTA_SERVIDORES]"
...
telnet_access_no_proxy ${SERVIDOR_CON_PUERTO}
uniforma_salida ${SALIDA_SITE_TELNET}
...
#-Insertamos un control de lanzamiento
...
echo "Inicio chequeo sites. Peticiones a puertos especificos"
linea_normal ${LONGITUD_LINEA}
echo "Fichero: ${RUTA_SERVIDORES} "
linea_normal ${LONGITUD_LINEA}
#---------------------------------------------------------------
#- Vamos a eliminar los puntos de lso servidores
