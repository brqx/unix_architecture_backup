...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E get_ssh_var_raiz ibm soa6 22 --> Acceso a soa6 con usuario ibm
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 
...
e_pmt get_ssh_var_raiz_rct "[usuario] [servidor] [puerto] [variable] "
...
ssh -p ${PUERTO_CONEXION}  ${USUARIO_CONEXION}@${IP_CONEXION} "bash -l -c '.  \${HOME}/r_clean.x > /dev/null 2> /dev/null   &&  echo \${RCT} ' "  > ./entorno_remoto_brqx.dat
