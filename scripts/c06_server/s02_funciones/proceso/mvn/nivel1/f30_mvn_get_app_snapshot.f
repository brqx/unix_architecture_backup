...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E mvn_get_library LIV1 11_2 -- Oteniene la version 11_2 de la libreria LIV1
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

#- Alias mvear
mvn_get_app_snapshot()
{
IS_mvn_get_app_snapshot="Obtiene el ear de maven"
INFO_SCRIPT=${IS_mvn_get_app_snapshot}
...
RUTA_NEXUS=${RUTA_NEXUS_SNAPSHOTS_APPS}
fi

if [ "${APLICACION}" == "-?" ]; then
## Sistema de auto documentacion de funciones

e_pmt mvn_get_app_snapshot "app [version]"
...
#-------------------------------------------------------------------
# wget http://nexus:8080/nexus/content/repositories/snapshots/com/mapfre/dgtp/aaa/taller/
#  AAA_TALLERARQOS_FEAR/0.0.1-SNAPSHOT/
#  AAA_TALLERARQOS_FEAR-0.0.1-20111220.230532-74.ear
#-------------------------------------------------------------------

echo "Se le pasa : ${RUTA_NEXUS}"

APP_TRANSFORMADA=$( echo $APLICACION ) # |  tr '_' '.'  
VERSION_TRANSFORMADA=$( echo $VERSION | tr '_' '.' )

RUTA_NEXUS_TRANSFORMADA=${RUTA_NEXUS}${APP_TRANSFORMADA}/${VERSION_TRANSFORMADA}-SNAPSHOT/
...
RUTA_BACKUPS_APPS=${HISTORICO_ARTEFACTOS_MAVEN}s/${FECHA_ACTUAL}/${APLICACION}/
