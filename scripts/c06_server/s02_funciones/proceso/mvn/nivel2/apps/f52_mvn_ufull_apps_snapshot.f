...
#+ Ejemplo : 
#-------------------------------------------------------------------
#E mvn_ufull_apps_snapshot 11_2 -- Oteniene la version 11_2 de la libreria LIV1
#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

#- Alias mvufull
mvn_ufull_apps_snapshot()
{
IS_mvn_ufull_apps_snapshot="Actualiza todos los artefactos desde servidor maven remoto"
INFO_SCRIPT=${IS_mvn_ufull_apps_snapshot}
...
if [ "${VERSION_UFULL}" == "" ]; then
	VERSION_UFULL=${VERSION_ARQOS_MVN_APPS_SNAPSHOT}
fi
...
e_pmt mvn_ufull_apps_snapshot "[version] [ruta]"
...
mvn_gfull_apps_snapshot  ${VERSION_UFULL}
